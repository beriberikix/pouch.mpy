...
/* Suppress platform default_RNG — natmod doesn't need it and -Werror
   rejects unused static functions.  We zero-init g_rng_function below.
   Pre-define the include guard to skip platform-specific.inc entirely,
   but still include types.h for platform detection and word size defs. */
#include "types.h"
...
struct uECC_Curve_t {
    ...
    /* Function pointer fields removed for MicroPython natmod compatibility.
       The natmod linker (mpy_ld.py) rejects .data.rel.local sections created
       by const structs with function pointers.  Since only secp256r1 is
       enabled, all dispatch goes through direct-call macros below. */
};

/* Forward declarations for direct-call dispatch (defined in curve-specific.inc) */
...
/* Direct-call macros replacing curve->func_ptr(...) dispatch */
#define CURVE_DOUBLE_JACOBIAN(curve, X1, Y1, Z1) double_jacobian_default((X1), (Y1), (Z1), (curve))
#define CURVE_X_SIDE(curve, result, x) x_side_default((result), (x), (curve))
#if uECC_SUPPORT_COMPRESSED_POINT
#define CURVE_MOD_SQRT(curve, a) mod_sqrt_default((a), (curve))
#endif
#if (uECC_OPTIMIZATION_LEVEL > 0)
#define CURVE_MMOD_FAST(result, product) vli_mmod_fast_secp256r1((result), (product))
#endif
...
/* Eliminate the RNG function pointer variable entirely for natmod.
   mpy_ld.py can't handle static BSS variables (fixed relocation to bss).
   We only use uECC_shared_secret() which doesn't need RNG.  Define as a
   compile-time zero so all if(g_rng_function) checks become dead code. */
#define g_rng_function ((uECC_RNG_Function)0)
...
#if (uECC_OPTIMIZATION_LEVEL > 0)
    CURVE_MMOD_FAST(result, product);
#else
...
static void XYcZ_initial_double(uECC_word_t * X1,
    ...
    CURVE_DOUBLE_JACOBIAN(curve, X1, Y1, z);
...
#endif
    CURVE_X_SIDE(curve, y, point);
    CURVE_MOD_SQRT(curve, y);
...
uECC_VLI_API int uECC_valid_point(const uECC_word_t *point, uECC_Curve curve) {
    ...
    CURVE_X_SIDE(curve, tmp2, point); /* tmp2 = x^3 + ax + b */
...
int uECC_verify(const uint8_t *public_key,
    ...
    for (i = num_bits - 2; i >= 0; --i) {
        uECC_word_t index;
        CURVE_DOUBLE_JACOBIAN(curve, rx, ry, z);
